...
const App = () => {
  ...
  const [screenInitiate, setScreenInitiate] = useState("login")
  ...
  useEffect(() => {
    ...
    axios.get("https://ygtechdev.my.id/dataProvinsi")
      .then((response) => {
  ...
  const handleProvinsiChange = (e, index) => {
    ...
    axios.post(`https://ygtechdev.my.id/filterKabKot`, {
  ...
  const handleKabupatenChange = (e, index) => {
    ...
    // Ambil data Kecamatan berdasarkan Kabupaten/Kota yang dipilih
    axios.post(`https://ygtechdev.my.id/filterKecamatan`, {
      kabkot_id: kabKotaId
    })
      .then((response) => {
  ...
  const handleKecamatanChange = (e, index) => {
    ...
    axios.post(`https://ygtechdev.my.id/filterKelurahan`, {
  ...
  const [userList, setUserList] = useState([dataLogins]);
  ...
  const handleChangeLogin = (index, e) => {
    const { name, value, files } = e.target;
    const newUserList = [...userList];

    // Cek apakah input adalah file
    if (files) {
      newUserList[index][name] = files[0]; // Simpan file pertama dari input
    } else {
      newUserList[index][name] = value; // Simpan nilai teks atau lainnya
    }
  ...
  const handleUpload = async (datas) => {
    ...
    try {
      ...
      const response = await axios.post("https://ygtechdev.my.id/upload", data, {
        headers: {
      ...
      if (response.status === 200 && response.data.fileUrl) {
        ...
        const sendMessageResponse = await axios.post("https://ygtechapi.web.id/sendMessageImage", {
          // remoteJid: "120363387652160802@g.us",
          "remoteJid": "120363390123597612@g.us",

          text: `${datas.namaLengkap} ${datas.agama == "Islam" ? datas.jenisKelamin == "Laki-Laki" ? "Bin" : "Binti" : ""} ${datas.agama == "Islam" ? datas.statusBapak == "Meninggal" ? datas.namaBapak + "(Alm)" : datas.namaBapak : ""}`,
  ...
  const handleLogin = () => {
    console.log('DDD', dataLogins);
    
    if(dataLogins.username == "superadmin" && dataLogins.password == "p45w0rd") {
      setScreenInitiate("first")
    } else {
      alert('Gagal')
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    const headerMessage = `*${greeting},*\nizin melaporkan pertemuan di ${formList[0].lokasiBertemu +
      " " +
    ...
    const formattedData = formList.map((data, index) => {
      ...
      console.log('jmlindex', data.total);
    ...
    await axios.post('https://ygtechapi.web.id/sendMessage', {
      "remoteJid": "120363390123597612@g.us",
      // remoteJid: "120363387652160802@g.us",
      ...
      await axios.post('https://ygtechapi.web.id/sendMessage', {
        ...
        const dataKirim = formList.map((item) => ({
          "formData": {
            ...
            "namaLengkap": item.namaLengkap + " " + (item.agama == "Islam" ? item.jenisKelamin == "Laki-Laki" ? "Bin" : "Binti" : "") + " " + (item.agama == "Islam" ? item.statusBapak == "Meninggal" ? item.namaBapak + "(Alm)" : item.namaBapak : ""), // Nama lengkap dari formData
            ...
            "tanggalLahir": item.tempatLahir + ", " + new Intl.DateTimeFormat('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(item.tanggalLahir)), // Tempat dan tanggal lahir yang diformat
        ...
        try {
          const response = await axios.post('https://ygtechdev.my.id/api/save-to-sheets', { formData: dataKirim });
                ...
                (
                  <>
                    ...
                    <TextField
                      ...
                      type="text"
                      name="username"
                      onChange={(e) => handleChangeLogin(index, e)}
                      fullWidth
                      margin="normal"
                    />
                     <TextField
                ...
                (
                  <>
                    ...
                    <Button
                      variant="contained"
                      color="primary"
                      onClick={() => setScreenInitiate("form")}
                      sx={{ mt: 2 }}
                    >
                      Lanjutkan
                    </Button>
